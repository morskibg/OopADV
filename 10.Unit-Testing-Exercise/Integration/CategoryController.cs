...
public class CategoryController
{
    ...
    public CategoryController(IEnumerable<string> names)
        : this()
    {
        foreach (var name in names) this.AddCategory(name);
    }

    public void AddCategory(string name)
    {
        if (this.categories.Any(c => c.Name == name || c.ChildCategories.Any(cc => cc.Name == name))) return;
    ...
    public void AddUser(ICategory category, IUser user)
    {
    ...
    public void RemoveCategory(string name)
    {
        ...
        if (categoryToRemove.Parent == null) this.categories.Remove(categoryToRemove);
        else categoryToRemove.Parent.RemoveChild(categoryToRemove.Name);
    }

    public void RemoveCategory(IEnumerable<string> names)
    {
        foreach (var categoryName in names) this.RemoveCategory(categoryName);
    }
    ...
    private void MooveChildrentCategoriesToParent(ICategory categoryToRemove)
    {
        ...
        {
            foreach (var category in categoryToRemove.ChildCategories) this.categories.Add(category);

            return;
        }

        foreach (var child in categoryToRemove.ChildCategories) categoryToRemove.Parent.AddChild(child);
    }

    private void RemoveCategoryFromUsersLists(ICategory categoryToRemove)
    {
        foreach (var user in categoryToRemove.Users) user.RemoveCategory(categoryToRemove);
    }
}